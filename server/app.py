...
def zhu_send_smtp_email(to_email: str, subject: str, body: str):
    ...
    if not smtp_host or not smtp_user or not smtp_password or not smtp_from:
        raise HTTPException(status_code=500, detail="SMTP 環境變數未設定完整")
...
@app.post("/notify-email")
@app.post("/api/admin/notify-email")
async def admin_notify_email(request: Request):
    ...
    subject = data.get("subject") or "ZHU STOCK APP 通知"
    body = data.get("body") or data.get("message") or "系統通知"
